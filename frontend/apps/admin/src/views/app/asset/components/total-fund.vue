...
const chartOption = computed(() => {
  const xData = assetStore.totalFundData.time?.map((t) =>
    dayjs(t).format('MM-DD HH:mm:ss')
  );
  const yData = assetStore.totalFundData.count?.map((item) => item?.toFixed(2));
...
const handleTimeChange = () => {
  ...
  assetStore.getTotalFundData(hours);
};

onMounted(() => {
  assetStore.getTotalFundData(24);
});
</script>
