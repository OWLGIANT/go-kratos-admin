...
// present.
func (s *SkipList) Get(key float64) (value ValueType, ok bool) {
...
func (s *SkipList) GetGreaterOrEqual(min float64) (actualKey float64, value ValueType, ok bool) {
...
func (s *SkipList) Update(key float64, value ValueType) {
	if value.Value == 0.0 && !s.IsOutOfOrderReceived {
		s.Delete(key, value)
	} else {
		s.Set(key, value)
	}
...
func (s *SkipList) Set(key float64, value ValueType) {
	if key == 0.0 {
	...
	// if candidate != nil && candidate.key == key {
	if candidate != nil && s.isEqual(candidate.key, key) {
		if candidate.value.Seq <= value.Seq {
...
func (s *SkipList) Delete(key float64, valueIn ValueType) (value ValueType, ok bool) {
	if key == 0.0 {
		panic("goskiplist: nil keys are not supported")
	}
	...
	// if candidate == nil || candidate.key != key {
	if candidate == nil || !s.isEqual(candidate.key, key) || candidate.value.Seq > valueIn.Seq {
...
func NewCustomMap(isOutOfOrderReceived bool, lessThan func(l, r float64) bool, isEqual func(l, r float64) bool) *SkipList {
	return &SkipList{
		lessThan: lessThan,
		isEqual:  isEqual,
		header: &node{
