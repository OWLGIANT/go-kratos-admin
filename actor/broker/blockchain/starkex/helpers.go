...
// apex 专用
func NonceByClientIdForApex(clientId string) *big.Int {
	h := sha256.New()
	h.Write([]byte(clientId))

	hex := string(helper.EncodeHex(h.Sum(nil)))[:8]
	a := new(big.Int)
	if res, err := HexToInt(hex); err == nil {
		a.SetInt64(int64(res))
	}
	return a
}
...
// @param hexStr without 0x prefix
...
func GetTransferErc20Fact(recipient string, tokenDecimals int, humanAmount, tokenAddress, salt string) (string, error) {
	// fmt.Println("GetTrnsferErc20Fact", recipient, tokenDecimals, humanAmount, tokenAddress, salt)
