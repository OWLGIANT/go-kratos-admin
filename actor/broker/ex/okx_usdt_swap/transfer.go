...
func (rs *OkxUsdtSwapRs) DoTransferMain(param transfer.TransferParams) (err helper.ApiError) {
	uri := "/api/v5/asset/transfer"
	reqBody := make(map[string]interface{})
	reqBody["type"] = strings.Split(param.TransferType, "-")[0]
	reqBody["ccy"] = "USDT"
	reqBody["from"] = strings.Split(param.TransferType, "-")[1]
	reqBody["to"] = strings.Split(param.TransferType, "-")[2]
	reqBody["amt"] = strconv.FormatFloat(param.Amount, 'f', -1, 64)
	reqBody["subAcct"] = param.SubAcctName
	...
	_, err.NetworkError = rs.client.post(uri, reqBody, func(respBody []byte, respHeader *fasthttp.ResponseHeader) {
		...
		if helper.GetStringFromBytes(value, "code") != "0" {
			err.HandlerError = fmt.Errorf("DoTransferMain err: %s, sub-acct: %s", value, param.SubAcctUid)
			return
		}
		log.Info("DoTransferMain success ,transId %v", value.GetArray("data")[0].Get("transId").String())
	})
...
func (rs *OkxUsdtSwapRs) DoTransferSub(param transfer.TransferParams) (err helper.ApiError) {
	uri := "/api/v5/asset/transfer"
	reqBody := make(map[string]interface{})
	reqBody["type"] = strings.Split(param.TransferType, "-")[0]
	reqBody["ccy"] = "USDT"
	...
	_, err.NetworkError = rs.client.post(uri, reqBody, func(respBody []byte, respHeader *fasthttp.ResponseHeader) {
		...
		if helper.GetStringFromBytes(value, "code") != "0" {
			err.HandlerError = fmt.Errorf("DoTransferSub err: %s, sub-acct: %s", value, param.SubAcctUid)
			return
		}
		log.Info("DoTransferSub success ,transId %v", value.GetArray("data")[0].Get("transId").String())
	})
...
func (rs *OkxUsdtSwapRs) DoTransferSubInner(params transfer.TransferParams) (err helper.ApiError) {
	...
	_, err.NetworkError = rs.client.post(uri, reqBody, func(respBody []byte, respHeader *fasthttp.ResponseHeader) {
		...
		value, err.HandlerError = p.ParseBytes(respBody)
		if err.HandlerError != nil {
			log.Errorf("DoTransferSubInner parse error %v", err)
			return
		}
		if helper.GetStringFromBytes(value, "code") != "0" {
			err.HandlerError = fmt.Errorf("DoTransferSubInner err: %s, sub-acct: %s", value, params.SubAcctUid)
			return
		}
		log.Info("DoTransferSubInner success ,transId %v", value.GetArray("data")[0].Get("transId").String())
	})

	if err.NetworkError != nil {
		log.Errorf("DoTransferSubInner network error %v", err)
		return
	}
	return
}

func (rs *OkxUsdtSwapRs) GetBalance(params transfer.GetBalanceParams) (balance float64, err helper.ApiError) {
...
func (rs *OkxUsdtSwapRs) GetMainBalance(params transfer.GetMainBalanceParams) (balance float64, err helper.ApiError) {
	if params.AccountType == "trade" {
		...
		_, err.NetworkError = rs.client.get(url, reqBody, true, func(respBody []byte, respHeader *fasthttp.ResponseHeader) {
			...
			if helper.GetStringFromBytes(value, "code") != "0" {
				err.HandlerError = fmt.Errorf("GetMainBalance err: %s", string(respBody))
				return
			}
			data := value.GetArray("data")[0].GetArray("details")
			balance = helper.MustGetFloat64(data[0], "availBal")
			log.Infof("get main trade acct balance: %f", balance)
		})
		return
	}
	...
	_, err.NetworkError = rs.client.get(url, reqBody, true, func(respBody []byte, respHeader *fasthttp.ResponseHeader) {
		...
		balance = helper.MustGetFloat64(data[0], "availBal")
		log.Infof("get main acct balance: %f", balance)
	})
	return
}

func (rs *OkxUsdtSwapRs) GetAssert(params transfer.GetBalanceParams) (accountsInfo []transfer.AccountInfo, err helper.ApiError) {
	url := "/api/v5/asset/subaccount/balances"
	reqBody := make(map[string]interface{})
	reqBody["subAcct"] = params.SubAcctName
	reqBody["ccy"] = "USDT"
	_, err.NetworkError = rs.client.get(url, reqBody, true, func(respBody []byte, respHeader *fasthttp.ResponseHeader) {
		...
		for _, v := range value.GetArray("data") {
			balance = balance + helper.MustGetFloat64(v, "bal")
		}
...
func (rs *OkxUsdtSwapRs) GetSubList() (accountsInfo []transfer.AccountInfo, err helper.ApiError) {
	p := handyPool.Get()
	defer handyPool.Put(p)
	var value *fastjson.Value
...
func (rs *OkxUsdtSwapRs) CreateSubAcct(params transfer.CreateSubAcctParams) (accountsInfo []transfer.AccountInfo, err helper.ApiError) {
	// OKX 不支持此功能
	return
}

func (rs *OkxUsdtSwapRs) CreateSubAPI(params transfer.APIOperateParams) (result transfer.Api, err helper.ApiError) {
	// OKX 不支持此功能
	return
}

func (rs *OkxUsdtSwapRs) ModifySubAPI(params transfer.APIOperateParams) (result transfer.Api, err helper.ApiError) {
	...
	_, err.NetworkError = rs.client.post(url, reqBody, func(respBody []byte, respHeader *fasthttp.ResponseHeader) {
		...
		if helper.GetStringFromBytes(value, "code") != "0" {
			...
			data := value.GetArray("data")
			result.Subuid = fmt.Sprintf("%s", params.UID)
...
func (rs *OkxUsdtSwapRs) DoTransferAllDireaction(params transfer.TransferAllDirectionParams) (err helper.ApiError) {
	// OKX 不支持此功能，返回空实现
	return
}

func (rs *OkxUsdtSwapRs) GetDepositAddress() (assress []transfer.WithDraw, err helper.ApiError) {
...
func (rs *OkxUsdtSwapRs) WithDraw(params transfer.WithDraw) (result string, err helper.ApiError) {
	...
	_, err.NetworkError = rs.client.post(uri, query, func(respBody []byte, respHeader *fasthttp.ResponseHeader) {
		p := handyPool.Get()
		defer handyPool.Put(p)
		var value *fastjson.Value
		value, err.HandlerError = p.ParseBytes(respBody)
...
func (rs *OkxUsdtSwapRs) WithDrawHistory(param transfer.GetWithDrawHistoryParams) (records []transfer.WithDrawHistory, err helper.ApiError) {
	...
	_, err.NetworkError = rs.client.get(uri, reqBody, true, func(respBody []byte, respHeader *fasthttp.ResponseHeader) {
		p := handyPool.Get()
		defer handyPool.Put(p)
		var value *fastjson.Value
		value, err.HandlerError = p.ParseBytes(respBody)
