...
// ZapLogger 默认会使用zap作为日志输出引擎. Log集成了日志切割的功能。默认文件大小100M，自动压缩
...
func (l *ZapLogger) Build() {
	...
	if l.Caller {
		caller := max(2, l.CallerSkip)
		l.logger = l.logger.WithOptions(zap.AddCaller(), zap.AddCallerSkip(caller))
	}
...
//	}
func (l *ZapLogger) ErrorfWithStacktrace(template string, args ...interface{}) {
	l.sugar.Errorf(template, args...)
	PrintStacktrace(l)
}
...
func (l *ZapLogger) FlushAndClose() error {
	return l.logger.Sync()
}
